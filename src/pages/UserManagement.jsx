...
function SortIcon({ sortKey, currentSort }) {
  ...
  return currentSort.dir === 'asc'
    ? <ChevronUp className="w-3 h-3 text-indigo-600 ml-0.5 inline" />
    : <ChevronDown className="w-3 h-3 text-indigo-600 ml-0.5 inline" />;
}
  ...
  const EmployeeForm = ({ form, setForm, onSubmit, isSaving, submitLabel }) => (
    <form onSubmit={onSubmit} className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
      ...
      <div className="border-b border-gray-200 pb-1 mb-2">
        <p className="text-xs font-bold text-indigo-600 uppercase tracking-wider">{lang === 'th' ? 'ข้อมูลส่วนตัว' : 'Personal Info'}</p>
      </div>
      ...
      <div className="border-b border-gray-200 pb-1 mb-2 mt-4">
        <p className="text-xs font-bold text-indigo-600 uppercase tracking-wider">{lang === 'th' ? 'ข้อมูลงาน' : 'Work Info'}</p>
      </div>
      ...
      {/* เงินเดือน & ธนาคาร */}
      <div className="border-b border-gray-200 pb-1 mb-2 mt-4">
        <p className="text-xs font-bold text-indigo-600 uppercase tracking-wider">{lang === 'th' ? 'เงินเดือน & ธนาคาร' : 'Salary & Bank'}</p>
      </div>
      <div className="grid grid-cols-4 gap-3">
      ...
      <div className="border-b border-gray-200 pb-1 mb-2 mt-4">
        <p className="text-xs font-bold text-indigo-600 uppercase tracking-wider">{lang === 'th' ? 'ประกันสังคม / ภาษี / กองทุน' : 'SSO / Tax / PVD'}</p>
      </div>
      <div className="grid grid-cols-3 gap-3">
      ...
      {/* ผู้ติดต่อฉุกเฉิน */}
      <div className="border-b border-gray-200 pb-1 mb-2 mt-4">
        <p className="text-xs font-bold text-indigo-600 uppercase tracking-wider">{lang === 'th' ? 'ผู้ติดต่อฉุกเฉิน' : 'Emergency Contact'}</p>
      </div>
      <div className="grid grid-cols-3 gap-3">
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col lg:flex-row gap-3 items-start lg:items-center justify-between">
        <div className="flex flex-col sm:flex-row gap-2 flex-1 w-full">
          ...
          <select value={filterRole} onChange={e => setFilterRole(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-white">
            <option value="all">{L.allRoles}</option>
            {ROLES.map(r => <option key={r.value} value={r.value}>{lang === 'th' ? r.labelTh : r.labelEn}</option>)}
          </select>
          <select value={filterDept} onChange={e => setFilterDept(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-white max-w-xs">
            <option value="all">{L.allDepts}</option>
            {uniqueDepts.map(d => <option key={d.id} value={d.id}>{lang === 'th' ? d.name_th : (d.name_en || d.name_th)}</option>)}
          </select>
          <select value={filterBU} onChange={e => setFilterBU(e.target.value)} className="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-white max-w-xs">
              ...
              <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-xl shadow-lg z-50 p-3 w-64 max-h-80 overflow-y-auto">
                ...
                {COLUMNS.map(col => (
                  <label key={col.key} className="flex items-center gap-2 py-1 cursor-pointer hover:bg-gray-50 rounded px-1">
                    <input type="checkbox" checked={!!visibleCols[col.key]} onChange={() => toggleColVisibility(col.key)} className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
          ...
          <table className="w-full text-sm whitespace-nowrap">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50">
                <th className="py-2.5 px-2 text-left w-8 sticky left-0 bg-gray-50/95 z-10">
                  <input type="checkbox" checked={selectAll} onChange={toggleSelectAll} className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                </th>
                {activeColumns.map(col => (
                  <th key={col.key} className={`py-2.5 px-2 text-left font-medium text-gray-500 text-[10px] uppercase tracking-wider cursor-pointer hover:text-indigo-600 select-none ${col.key === 'employee_code' ? 'sticky left-8 bg-gray-50/95 z-10' : ''}`}
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={emp.id} className={`border-b border-gray-50 hover:bg-gray-50 transition-colors ${selectedIds.has(emp.id) ? 'bg-indigo-50/50' : ''}`}>
                  <td className="py-1.5 px-2 sticky left-0 bg-white z-10">
                    <input type="checkbox" checked={selectedIds.has(emp.id)} onChange={() => toggleSelect(emp.id)} className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                  </td>
                  {activeColumns.map(col => (
                  ...
                  <td className="py-1.5 px-2">
                    <select value={emp.system_role} onChange={e => handleRoleChange(emp.id, e.target.value)} disabled={saving[emp.id]}
                      className={`text-[10px] font-semibold rounded-md px-2 py-1 border-0 cursor-pointer focus:ring-2 focus:ring-indigo-500 outline-none appearance-none pr-5 ${
                  ...
                  <td className="py-1.5 px-2 text-center">
                    <div className="flex items-center justify-center gap-0.5">
                      <button onClick={() => openEditModal(emp)} className="p-1 rounded-lg hover:bg-indigo-50 text-gray-400 hover:text-indigo-600 transition-colors" title={lang === 'th' ? 'แก้ไข' : 'Edit'}>
