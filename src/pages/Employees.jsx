...
export default function Employees({ lang }) {
  ...
  const handleExportCSV = () => {
    ...
    const STATUS_LABEL = { active: 'ทำงานอยู่', probation: 'ทดลองงาน', resigned: 'ลาออก', terminated: 'พ้นสภาพ' };
    ...
    const exportRows = filteredEmployees.map((emp) => ({
      ...
      'สถานะ': STATUS_LABEL[emp.status] || emp.status || '',
  ...
  // Calculate KPI metrics (must be before any early return to keep hooks order consistent)
  // Apply global company filter first, then local filters
  ...
  const filteredEmployees = useMemo(() => {
    return companyFilteredEmployees.filter((emp) => {
  ...
  }, [companyFilteredEmployees, searchTerm, filterDept, filterStatus, filterType]);
  ...
  return (
    <div className="space-y-5">
              ...
              <thead>
                <tr className="border-b border-gray-200 bg-gray-50">
                  {allColumns.filter(c => visibleColumns.includes(c.key)).map(col => (
                    <th key={col.key} className="px-4 py-3 text-left font-semibold text-gray-700 whitespace-nowrap">{col.label}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filteredEmployees.map((emp) => {
                  ...
                  const cellMap = {
                    ...
                    status: <td key="status" className="px-4 py-3"><StatusBadge status={emp.status} labels={statusLabels} /></td>,
      ...
      {selectedEmployee && (() => {
        ...
        const dept = departments.find(d => d.id === emp.department_id);
        const deptName = dept ? (lang === 'th' ? dept.name_th : dept.name_en) : '-';
        ...
        const statuses = { active: 'ทำงานอยู่', probation: 'ทดลองงาน', resigned: 'ลาออก', terminated: 'พ้นสภาพ' };
        ...
        const SectionLabel = ({ title, color = 'indigo' }) => (
          <div className={`border-b-2 border-${color}-200 pb-1 mb-3`}>
              ...
              <div className="flex items-center gap-4 px-6 py-5 border-b border-gray-100 bg-gradient-to-r from-[#f0fce8] to-[#E6F9F0] rounded-t-2xl">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex gap-2 mt-1.5">
                    <StatusBadge status={emp.status} labels={statuses} />
              ...
              <div className="overflow-y-auto flex-1 p-6 space-y-5">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-x-8">
                    ...
                    <InfoRow label="สถานะ" value={statuses[emp.status] || emp.status} />
